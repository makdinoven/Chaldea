...
### RBAC — обязательные тесты

- При изменении admin-эндпоинтов: тестировать доступ для admin, moderator, editor, user ролей
- При добавлении новых разрешений: проверить, что админ автоматически получает новое разрешение
- Всегда тестировать 403 для неавторизованных ролей
- Проверять, что `get_admin_user()` принимает admin и moderator, отклоняет editor и user
