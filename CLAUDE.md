...
8. **Frontend стили — миграция на Tailwind CSS** (см. T1 в `docs/ISSUES.md`):
   - **Новые компоненты** — писать сразу на Tailwind, без CSS/SCSS-файлов.
   - **Изменение стилей существующего компонента** — мигрировать весь компонент на Tailwind в том же PR, удалить старый CSS/SCSS.
   - **Задача не касается стилей** — не трогать, оставить как есть.
   - Tailwind и старые SCSS сосуществуют в переходный период.
9. **Frontend язык — миграция на TypeScript** (см. T3 в `docs/ISSUES.md`):
   - **Новые файлы** — писать сразу на TypeScript (`.tsx` / `.ts`).
   - **Изменение логики существующего файла** — мигрировать на TypeScript в том же PR (`.jsx` -> `.tsx`), добавить типы.
