...
def _can_view(db, character_id, user):
    return visibility.can_view_private(db, character_id, user)


def _require(db, character_id, user):
    return visibility.require_private_access(db, character_id, user)
...
class TestPlayerCharacter:
    ...
    def test_matrix(self, db, viewer, expected, why):
        assert _can_view(db, PLAYER_CHARACTER, viewer) is expected, why

    def test_role_and_permission_is_an_and_not_an_or(self, db):
        """The single assertion that catches `or` creeping into a copy."""
        assert _can_view(db, PLAYER_CHARACTER, MODERATOR_NO_PERM) is False
        assert _can_view(db, PLAYER_CHARACTER, EDITOR_WITH_PERM) is False
        assert _can_view(db, PLAYER_CHARACTER, MODERATOR) is True

    def test_the_owner_is_matched_by_user_id_not_by_name(self, db):
        ...
        assert _can_view(db, PLAYER_CHARACTER, impostor) is False
...
class TestNpcCharacter:
    ...
    def test_npc_is_public_for_everyone(self, db, viewer):
        assert _can_view(db, NPC_CHARACTER, viewer) is True

    def test_require_does_not_raise_for_an_npc(self, db):
        assert _require(db, NPC_CHARACTER, GUEST) is None
...
class TestRequirePrivateAccess:
    ...
    @pytest.mark.parametrize("viewer", [GUEST, STRANGER, MODERATOR_NO_PERM])
    def test_strangers_get_403_with_the_shared_russian_message(self, db, viewer):
        with pytest.raises(HTTPException) as exc:
            _require(db, PLAYER_CHARACTER, viewer)
    ...
    def test_privileged_viewers_pass_through(self, db, viewer):
        assert _require(db, PLAYER_CHARACTER, viewer) is None
