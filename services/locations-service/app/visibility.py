...
Async twin for locations-service (AsyncSession + `await db.execute`), added by the
FEAT-171 review fix for the NPC-shop side door: this service proxies private
attributes with `X-Internal-Token`, so it needs the same viewer predicate the
owning services use.
