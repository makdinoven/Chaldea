"""Add mobs:manage permission for mob system admin endpoints
...
def upgrade() -> None:
    ...
    if existing:
        ...
        conn.execute(
            ...
            {"m": "mobs", "a": "manage", "d": "Управление шаблонами мобов, спавном и активными мобами"}
        )
...
def downgrade() -> None:
    conn = op.get_bind()
    rows = conn.execute(
        sa.text("SELECT id FROM permissions WHERE module = 'mobs' AND action = 'manage'")
    ).fetchall()
