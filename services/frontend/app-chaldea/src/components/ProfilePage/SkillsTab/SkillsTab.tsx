...
// perk system from FEAT-125. FEAT-166: one PanelShell with toolbar + scroll
// grid, shared card/frame/state primitives, detail dialog via ModalShell.
...
const PANEL_TITLE = 'Навыки';
const PANEL_ICON = <Sparkles size={18} strokeWidth={1.8} className="text-gold shrink-0" />;
...
const SkillsTab = ({ characterId }: SkillsTabProps) => {
  ...
  // Loading / fatal error render inside the same panel, so the layout doesn't jump.
  if (loading || (error && skills.length === 0)) {
    return (
      ...
      >
        <PanelShell title={PANEL_TITLE} icon={PANEL_ICON} className={PANEL_DESKTOP_HEIGHT_CLASS}>
          {loading ? <LoadingState /> : <ErrorState message={error ?? ''} />}
        </PanelShell>
      </motion.div>
    );
  }

  const perkPointsChip: ReactNode =
    totalPerkPoints > 0 ? (
      <span className="flex items-center gap-2 px-3 py-1 rounded-full border border-gold/30 bg-gold/[0.08] whitespace-nowrap">
        <span className="w-2 h-2 rounded-full bg-gold shadow-[0_0_8px_rgba(240,217,92,0.8)]" />
        <span className="text-xs font-medium text-gold">{perkPointsText(totalPerkPoints)}</span>
      </span>
    ) : null;
  ...
  return (
    ...
    >
      <PanelShell
        ...
        headerExtra={
          <div className="flex items-center gap-3">
            {perkPointsChip && <div className="hidden sm:flex">{perkPointsChip}</div>}
            <PanelCounter>
      ...
      >
        <PanelToolbar>
          {perkPointsChip && <div className="flex sm:hidden w-full">{perkPointsChip}</div>}
          <FilterChips
            items={TYPE_FILTERS}
            active={typeFilter}
          ...
          <Link
            to="/skill-tree"
            className="btn-blue flex items-center justify-center gap-2 px-[18px] py-2.5 text-xs font-medium tracking-[0.04em] w-full sm:w-auto"
          >
        ...
        <PanelScrollArea>
          ...
          {skills.length === 0 ? (
            ...
            <motion.div
              ...
              className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-3.5"
            >
                ...
                return (
                  ...
                  >
                    ...
                    <GoldIconFrame
                      ...
                      src={skillImage}
                      alt={skillName}
                      fallback={<FallbackIcon size={26} strokeWidth={1.6} className="text-gold/70" />}
                    />
      ...
      {/* Detail dialog — existing resolved-skill wiring; the name lives inside the card */}
      <ModalShell
        open={selectedSkill !== null}
        onClose={() => setSelectedSkill(null)}
        size="md"
        bodyClassName="px-4 pb-4 pt-12 sm:px-6 sm:pb-6"
      >
        {detailLoading && !resolved && <LoadingState size="sm" />}

        {!detailLoading && !resolved && error && <ErrorState message={error} className="py-6" />}

        {resolved && <ResolvedSkillCard resolved={resolved} skill={skillWithPerks} />}
