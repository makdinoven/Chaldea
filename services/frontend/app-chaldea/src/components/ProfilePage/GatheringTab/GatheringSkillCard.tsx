 ...
 * mock (FEAT-151), moved onto the shared profile primitives (FEAT-166).
 * Renders a 52px category icon in a gold-gradient frame,
...
const GatheringSkillCard = ({ skill }: GatheringSkillCardProps) => {
  ...
  // Max rank (or unknown requirement) renders as a full bar, as before
  const barFull = skill.is_max_rank || requiredXp <= 0;
  const barValue = barFull ? 1 : skill.experience;
  const barMax = barFull ? 1 : requiredXp;
  ...
  return (
    <ProfileCard className="p-4 sm:p-5 flex flex-col gap-4 h-full">
      {/* Header: 52px icon frame + name + rank */}
      <div className="flex items-center gap-3.5">
        <GoldIconFrame size={52} shape="square">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <ProgressBar value={barValue} max={barMax} variant="gold" />
      </div>
