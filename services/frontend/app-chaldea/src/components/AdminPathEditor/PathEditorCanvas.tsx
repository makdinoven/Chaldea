...
const PathEditorCanvas = ({
  ...
  // Build position map for locations
  ...
  // Find which location/district was clicked (returns id or null)
  const findClickedMarker = useCallback((px: number, py: number): { type: 'location' | 'district'; id: number } | null => {
  ...
  }, [mode, clientToPercent, findClickedMarker, districts, onDrawClick, drawStartId, onDrawWaypointAdd, findClosestEdge, onSelectEdge, onDeleteEdge]);
  ...
  const renderDrawingPath = () => {
    ...
    const from = positionMap.get(drawStartId);
    if (!from) return null;
