...
const useBattleWebSocket = (
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      const wsUrl = `${protocol}//${window.location.host}/battles/ws/${battleId}?token=${wsToken}`;
      ...
      ws.onclose = (event: CloseEvent) => {
        ...
        // fatal refresh we just stop and fall back to polling; logout is
        // decided only by the HTTP layer's handleAuthFailure.
