...
### Баг: относительное время постов считается без учёта часового пояса — «только что» вместо реального возраста
**Сервис:** frontend
**Файл:** `services/frontend/app-chaldea/src/components/pages/LocationPage/PostCard.tsx` (`formatRelativeTime`)
**Обнаружено:** FEAT-159 (Frontend Dev, задача T5, 2026-09-13). Предсуществующий баг, к фиче отношения не имеет.
**Описание:** `posts.created_at` — наивный MySQL `TIMESTAMP`, записанный серверным `NOW()` (контейнеры в UTC) и сериализуемый без смещения: `"2026-03-23T10:23:58"` (проверено на живом `GET /locations/28/client/details`). `new Date(...)` трактует такую строку как **локальное** время браузера, поэтому у игрока в UTC+3 свежий пост оказывается «в будущем» и показывается как «только что», а пост трёхчасовой давности — как только что написанный. Затронут весь список постов.
**Почему не исправлено в FEAT-159:** задача T5 не касается отображения относительного времени; для собственной проверки «часового окна редактирования» добавлен локальный хелпер `parseServerDate`, который дочитывает `Z` к строкам без смещения. Исправление `formatRelativeTime` — отдельная правка, затрагивающая вид всей ленты.
**Возможное решение:** вынести `parseServerDate` в общий модуль (`utils/`) и перевести на него `formatRelativeTime` и остальные места, читающие наивные серверные таймстемпы.

### Долг: `action_gates.created_at` никто не читает, а цели гейта не валидируются при создании поста
**Сервис:** locations-service
**Файлы:** `services/locations-service/app/models.py:196` (`ActionGate.created_at`), `services/locations-service/app/main.py:772-792` (`_validate_intent_post`), `services/locations-service/app/crud.py:1006` (`create_action_gates`), точки вызова — `main.py:856` (`POST /locations/posts/`) и `main.py:1285` (`move_and_post`)
...
**Сервис:** locations-service
**Файлы:** `services/locations-service/app/main.py:2169-2187` (`PUT /admin/moderation/deletion-requests/{id}/review`), `services/locations-service/app/main.py:2189-2207` (`PUT /admin/moderation/reports/{id}/review`)
...
**Возможное решение:** либо объявить отдельную узкую схему `PostModerationDecisionRead` (7 полей), либо заполнять тело теми же данными, что и очередь.
