...
- `inventory-service:8004` -> POST `/inventory/internal/characters/{cid}/items` (+ `X-Internal-Token`, FEAT-167: лут локации, покупка у NPC, награда за квест — раньше был открытый `/inventory/{cid}/items` с проброшенным токеном игрока), POST `/inventory/internal/characters/{cid}/free_slots_check` (preflight на старте), POST `/inventory/internal/characters/{cid}/gathering/award` (атомарный award на finalize: ресурс + XP + ранг + прочность инструмента), GET `/inventory/characters/{cid}/gathering-skills` (ранговые бонусы для расчёта effective_*), GET `/inventory/internal/characters/{cid}/xp-multiplier` (+ `X-Internal-Token`, FEAT-168 #6: книга опыта за отыгрыш и за задания)
