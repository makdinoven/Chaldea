...
> **FEAT-170:** блок `INTERNAL_SERVICE_TOKEN` + `_internal_token_headers()` + `verify_internal_token` поднят в **начало** `main.py` (сразу после импортов и `OAUTH2_SCHEME_OPTIONAL`), на старом месте оставлена хлебная крошка. Причина: `Depends(verify_internal_token)` в декораторе маршрута вычисляется на импорте, поэтому любой internal-маршрут выше определения зависимости ронял контейнер с `NameError` на старте. Блок намеренно остался в `main.py`, а не вынесен в отдельный модуль — тесты (`test_internal_auth.py`, `test_internal_headers.py`, `test_gathering.py`, `test_character_left_location.py`) подменяют именно `main.INTERNAL_SERVICE_TOKEN`. **Не переносить блок обратно вниз.**
...
| GET | `/locations/quests/internal/check-completed` | FEAT-170. Завершил ли персонаж квест X. Вызывающий — `perk_evaluator._fetch_quest_completed` в character-attributes-service (ошибку глотает и возвращает `False`, то есть без заголовка перки молча перестали бы открываться) |
...
- `battle-pass-service:8012` -> POST `/battle-pass/internal/track-event` (+ `X-Internal-Token` с FEAT-170; два места: обычное и быстрое перемещение, событие `location_visit`). Fire-and-forget под `except: pass` — без заголовка прогресс боевого пропуска встал бы молча
