...
### Боевые эффекты расходников (FEAT-168)

Общий разбор боевой настройки предмета живёт в `src/utils/itemEffects.ts` — один
модуль на бой и на инвентарь, чтобы описания нигде не разошлись. Все поля
необязательные: бой, начатый до фичи, хранит быстрые слоты без них.

- **Журнал боя** (`BattlePageBar.tsx`, `formatBattleEvent`) — ветки для
  `item_use` (восстановление, включая выносливость, и остаток стопки),
  `item_rejected` (яд уже на оружии), `weapon_coating_applied` /
  `weapon_coating_expired`, `effects_removed`, `item_broken`, а также урон от
  предмета (`source_kind: "item"`). Запасной перевод никогда не печатает
  английское имя события.
- **Быстрые слоты** (`SkillPicker.tsx`) — под каждым предметом видно, что он
  делает, сколько осталось; второй яд отключён, пока действует прежний
  (`participants[pid].weapon_coating`).
- **Админка предметов** — `ItemEffectSections.tsx` (урон, эффекты, очищение, яд
  на оружие) и `ItemXpBuffsEditor.tsx` (до 8 строк ускорения опыта). Словарь
  эффектов импортируется из `AdminSkillsPage/skillConstants.ts`, не копируется.
  Сообщения проверок дословно совпадают с сообщениями inventory-service.
- **Инвентарь** — блоки «В бою» и «Ускорение опыта» в `ItemDetailModal`,
  краткая сводка в подсказке ячейки, подписи всех 8 видов опыта в
  `ActiveBuffIndicator`.
